...
and Python 2.7 (tested Centos 7, Ubuntu 14.04 LTS,
Ubuntu 15 IoT build, OS X El Capitan, and Windows 10 x64.
...
class Field():
    ...
    kWh_Tariff_4              Power in timeslot 3
    Rev_kWh_Tot               Meter rev. total
    Rev_kWh_Tariff_1          Rev in timeslot 1
    Rev_kWh_Tariff_2          Rev in timeslot 1
    Rev_kWh_Tariff_3          Rev in timeslot 1
    Rev_kWh_Tariff_4          Rev in timeslot 1
    ...
    RMS_Watts_Ln_1            V4, Direction is sign
    RMS_Watts_Ln_2            Direction has sign
    RMS_Watts_Ln_3            Direction has sign
    RMS_Watts_Tot             Direction has sign
    Cos_Theta_Ln_1            Cos Theta Line 1
    Cos_Theta_Ln_2            Cos Theta Line 2
    Cos_Theta_Ln_3            Cos Theta Line 3
    Max_Demand                Max demand value
    Max_Demand_Period         MaxDemandPeriod set
    Meter_Time                Seek splitEKMDate
    CT_Ratio                  CTRatio value
    Pulse_Cnt_1               Pulse Count Line 1
    Pulse_Cnt_2               Pulse Count Line 2
    Pulse_Cnt_3               Pulse Count Line 3
    Pulse_Ratio_1             PulseOutput Line 1
    Pulse_Ratio_2             PulseOutput Line 2
    Pulse_Ratio_3             PulseOutput Line 2
    State_Inputs'             ---
    ...
    kWh_Ln_1                  Line one power
    kWh_Ln_2                  Line one power
    kWh_Ln_3                  Line one power
    Rev_kWh_Ln_1              Line one power
    Rev_kWh_Ln_2              Line one power
    Rev_kWh_Ln_3              Line one power
    Resettable_kWh_Tot        Meter::maxDemandReset()
...
class Pulse():
    """ As passed to :func:`~ekmmeters.V4Meter.setPulseRatio`
...
class Meter(object):
        ...
        for fld in def_buf:
            ...
            try:  # scrub up messes on a field by field basis
                ...
                if fld_type == FieldType.Float:
                    ...
                    pf_val_str = str(raw_data)[1:]
                    pf_val_float = float(pf_val_str)
                    pf_type_str = str(raw_data)[:1]
                    if (pf_type_str == "C") and (pf_val_float != 0):
                        pf_val_float *= -1
                    pf_float = pf_val_float / 100
                    pf_string = str(pf_float)
                    def_buf[fld][MeterData.StringValue] = pf_string
                    def_buf[fld][MeterData.NativeValue] = pf_float
                else:
    ...
    def setCTRatio(self, new_ct, password="00000000"):
        ...
        try:
            self.clearCmdMsg()
            if ((new_ct != 100) and (new_ct != 200) and (new_ct != 400) and (new_ct != 600) and (new_ct != 800) and
                    (new_ct != 1000) and (new_ct != 1200) and (new_ct != 1500) and (new_ct != 200) and
                    (new_ct != 3000) and (new_ct != 4000) and (new_ct != 5000)):
...
class V3Meter(Meter):
    ...
    def initWorkFormat(self):
        ...
        self.m_blk_a["reserved_11"] = [20, FieldType.Hex, ScaleType.No, "", 0, False, False]
    ...
    def calculateFields(self):
        """v3 calculated fields (just power factor formatting for v3). """
        pf1 = self.m_blk_a[Field.Cos_Theta_Ln_1][MeterData.NativeValue]
        pf2 = self.m_blk_a[Field.Cos_Theta_Ln_2][MeterData.NativeValue]
        pf3 = self.m_blk_a[Field.Cos_Theta_Ln_3][MeterData.NativeValue]

        pf1_original = pf2_original = pf3_original = 0
        if pf1 < 0:
            pf1_original = int(((pf1 + 1) + 1) * 100)
        else:
            pf1_original = int(pf1 * 100)

        if pf2 < 0:
            pf2_original = int(((pf2 + 1) + 1) * 100)
        else:
            pf2_original = int(pf2 * 100)

        if pf3 < 0:
            pf3_original = int(((pf3 + 1) + 1) * 100)
        else:
            pf3_original = int(pf3 * 100)
...
class V4Meter(Meter):
    ...
    def calculateFields(self):
        ...
        pf1 = self.m_blk_b[Field.Cos_Theta_Ln_1][MeterData.NativeValue]
        pf2 = self.m_blk_b[Field.Cos_Theta_Ln_2][MeterData.NativeValue]
        pf3 = self.m_blk_b[Field.Cos_Theta_Ln_3][MeterData.NativeValue]
        ...
        pulse_1_cnt = self.m_blk_a[Field.Pulse_Cnt_1][MeterData.NativeValue]
        pulse_2_cnt = self.m_blk_a[Field.Pulse_Cnt_2][MeterData.NativeValue]
        pulse_3_cnt = self.m_blk_a[Field.Pulse_Cnt_3][MeterData.NativeValue]
    ...
    def setPulseRatio(self, line_in, new_cnst, password="00000000"):
